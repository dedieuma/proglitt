...
```{bash}
ls /home/andreas/Documents/proglitt/TD3/data/PP_size*.csv
```
...
```{r}
df1 <- read.csv("data/PP_size_3.csv");
head(df1);
```

```{r}
set.seed(42);
times_test <- rnorm(n = 50, mean = 4, sd = 1.5);
write.csv(times, "pp-synthetic.csv", row.names = FALSE);
```
...
```{r, fig.width=5}
hist(df1$time, breaks=1000000, xlab="Time (seconds)", main="Histogram OF MOI AHAHAHAH")
```
